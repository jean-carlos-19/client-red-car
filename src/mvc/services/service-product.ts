...
class ServiceProduct implements ProductController {
    ...
    public showCategory = async ({ category }: { category: string }): Promise<AxiosResponse<{ data: ProductDto[] }>> => {
    ...
    public search = async ({ search }: { search: Search }): Promise<AxiosResponse<{ data: ProductDto[] }>> => {
        return await this.http.get<ProductDto>(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/search/product?search=${search.search}`,
        );
    };
    public searchLaboratory = async (
        { category, laboratory, search }: {
            category: string,
            laboratory: string,
            search: Search,
        }
